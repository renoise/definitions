...
* Return types for main class constructors (`renoise.app()`, `.tool()`, `.song()` etc.) should be specified as an instance to allow the LSP to warn when trying to access non-constant properties on the classes themselves (like `renoise.Song.selected_track`), see `ViewBuilder` in [view_builder.lua](library/renoise/view_builder.lua) for a work-around.
